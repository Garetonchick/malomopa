...
//     assign существующего заказа - 400, (теперь 200)
...
func TestLogic(t *testing.T) {
	...
	t.Run("Invalid scenarios", func(t *testing.T) {
		...
		t.Run("Assign non-existent order", func(t *testing.T) {
			code, err := client.AssignOrder("non-existent", "52")
			require.NoError(t, err)
		...
		t.Run("Cancel after acquire", func(t *testing.T) {
			...
			resp, err := client.CancelOrder("41")
			require.NoError(t, err)
		...
		t.Run("Acquire after cancel", func(t *testing.T) {
			...
			resp, err := client.AcquireOrder("44")
			require.NoError(t, err)
			require.Equal(t, http.StatusBadRequest, resp.Code)
		})
	})
